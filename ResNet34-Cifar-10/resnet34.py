...
def resnet34(in_tensor, is_training):
    ...
    "中间层不变"
    tensor_go_next = repeat_basic_block(tensor_go_next, 64,  64, layer_num=1, repeat_times=3, is_training=is_training)
    print('\nblock1 finished')

    tensor_go_next = repeat_basic_block(tensor_go_next, 64, 128, layer_num=2, repeat_times=4, is_training=is_training)
    print('\nblock2 finished')

    tensor_go_next = repeat_basic_block(tensor_go_next, 128, 256, layer_num=3, repeat_times=6,  is_training=is_training)
    print('\nblock3 finished')

    tensor_go_next = repeat_basic_block(tensor_go_next, 256, 512, layer_num=4, repeat_times=3, is_training=is_training)  #(4,4,512)
    print('\nblock4 finished')

    "ave pool 层 修改，使得维度匹配"
...
flags = tf.flags
FLAGS=flags.FLAGS
flags.DEFINE_string('epoch', '300', 'train epoch' )
...
logits, output = resnet34(x, is_training)
...
with tf.Session(config=config) as sess:
    ...
    try:
        ...
        # saver.save(sess, ckpt_root_path+'/resnet18-raw', global_step=0, write_meta_graph=True)

        '''Training'''
        num_lr = 0.1
        with open(log_path, 'a') as f:
            f.write('time:2020/10/29  Reset34 for Cifar10 \n\n')
        ...
        for j in range(epoch):
            ...
            if (j+1)%5==0:
                ...
                if j==0:
                    saver.save(sess, ckpt_root_path+'/resnet34-'+str(test_acc)[0:7], global_step=j+1, write_meta_graph=True)
                else:
                    saver.save(sess, ckpt_root_path+'/resnet34-'+str(test_acc)[0:7], global_step=j+1, write_meta_graph=False)
    finally:
            coord.request_stop()
            coord.join(threads)
